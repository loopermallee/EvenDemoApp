...
object SpeechBridge {
    ...
    fun init(activity: Activity, onResultCallback: (String?, Boolean) -> Unit) {
        ...
        if (speechRecognizer == null) {
            ...
            speechRecognizer?.setRecognitionListener(object : RecognitionListener {
                ...
                override fun onError(error: Int) { isListening = false; onResult?.invoke("", true) }
                ...
                override fun onPartialResults(partialResults: Bundle?) {
                    ...
                    if (!text.isNullOrEmpty()) onResult?.invoke(text, false)
                }
    ...
    fun start(activity: Activity, done: (ok: Boolean, err: String?) -> Unit) {
        ensureAudioPermission(activity) { granted ->
            if (!granted) { done(false, "Microphone permission denied"); return@ensureAudioPermission }
            if (!SpeechRecognizer.isRecognitionAvailable(activity)) { done(false, "Speech recognition not available"); return@ensureAudioPermission }
            if (isListening) { done(true, null); return@ensureAudioPermission }
            init(activity) { text, isFinal -> onResult?.invoke(text, isFinal) }
            isListening = true
            try { speechRecognizer?.startListening(recognizerIntent); done(true, null) }
            catch (e: Exception) { isListening = false; done(false, e.message) }
        }
    }

    fun stop(finalize: Boolean) {
        if (!isListening && speechRecognizer == null) return
        try { if (finalize) speechRecognizer?.stopListening() else speechRecognizer?.cancel() } catch (_: Exception) {}
